...
const handler = async (m, {conn, text, usedPrefix, command}) => {
  ...
  if (!text) throw `*bego*\n\n*Cnth :*\n*◉ ${usedPrefix + command} *Cara Membuat Anak*\n*◉ ${usedPrefix + command} AI adalah*`;
  m.react('🕘');
  try {
      ...
      try {
        ...
        const replacementString2 = ' Indonesia ';
      ...
      try {
        ...
        const akuariapiresult2 = await translate(`${akuariapijson2.respon}`, {to: 'id', autoCorrect: true});
      ...
      try {
        ...
        const akuariapiresult1 = await translate(`${akuariapijson1.respon}`, {to: 'id', autoCorrect: true});
        m.reply(`${akuariapiresult1.text}`.trim());
    } catch {
        throw `*Err server down*`;
     }}
    }}
   }}
  }}
...
handler.command = /^(openai|chatgpt|ai|robot|openai2|chatgpt2|ia2|robot2|Mystic|MysticBot)$/i;
